        ...
        <div id="quizForm" class="{{ session('quiz_result_id') && !session('error') ? 'hidden' : '' }}">
            <form action="{{ route('modules.quiz.submit', $module) }}" method="POST" onsubmit="return handleQuizSubmit(event)">
                ...
                @php
                    ...
                    $groupCount = $questionGroups->count();
                @endphp
                ...
                <div class="mb-8 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-200">
                    <h4 class="text-lg font-bold text-gray-800 mb-4 text-center">Navigation des questions</h4>
                    <div class="flex flex-wrap justify-center gap-3">
                        @foreach($questionGroups as $groupIndex => $group)
                        <button type="button" 
                                onclick="showQuestionGroup({{ $groupIndex }})" 
                                class="question-group-btn px-4 py-2 rounded-xl font-medium transition-all duration-300 {{ $groupIndex === 0 ? 'bg-blue-500 text-white shadow-md' : 'bg-white text-gray-700 border border-blue-200 hover:bg-blue-100' }}"
                                id="group-btn-{{ $groupIndex }}">
                            Partie {{ $groupIndex + 1 }} ({{ $group->count() }} questions)
                        </button>
                        @endforeach
                    </div>
                </div>
                ...
                @foreach($questionGroups as $groupIndex => $group)
                <div class="question-group {{ $groupIndex === 0 ? '' : 'hidden' }}" id="question-group-{{ $groupIndex }}">
                    <div class="mb-6 flex items-center justify-between">
                        <h4 class="text-xl font-bold text-gray-800">
                            @if($groupCount > 1)
                            Partie {{ $groupIndex + 1 }} sur {{ $groupCount }}
                            @else
                            Questions du quiz
                            @endif
                        </h4>
                        <span class="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
                            {{ $group->count() }} question(s)
                        </span>
                    </div>
                    ...
                    <div class="space-y-8">
                        ...
                        @php
                            $globalIndex = ($groupIndex * 0) + $index;
...
<!-- SECTION CORRECTION DÉTAILLÉE - EN DEHORS DU BLOC TEST DE VALIDATION -->
...
<div id="resultsSection" class="mb-8">
    <div class="bg-white rounded-3xl shadow-xl border border-gray-200 overflow-hidden">
        <div class="bg-gradient-to-r from-green-500 to-emerald-600 p-8 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-green-100 text-lg">Cliquez sur chaque question pour voir l'analyse complète</p>
                </div>
                <div class="hidden lg:block">
                    <div class="bg-white bg-opacity-20 rounded-xl p-4 text-center">
                        <div class="text-2xl font-bold">{{ session('quiz_total_questions') }}</div>
                        <div class="text-sm opacity-90">Questions</div>
                    </div>
                </div>
            </div>
        ...
        <div class="p-8">
            ...
            <div class="mt-8 p-6 bg-gradient-to-r from-green-500 to-emerald-600 rounded-2xl text-white shadow-lg">
...
// Fonction pour afficher un groupe de questions spécifique
function showQuestionGroup(groupIndex) {
    ...
    const targetGroup = document.getElementById(`question-group-${groupIndex}`);
    if (targetGroup) {
        targetGroup.classList.remove('hidden');
    }
    
    // Mettre à jour les boutons de navigation
    document.querySelectorAll('.question-group-btn').forEach((btn, index) => {
        if (index === groupIndex) {
            ...
            btn.classList.add('bg-white', 'text-gray-700', 'border', 'border-blue-200', 'hover:bg-blue-100');
        }
    });
    
    // Faire défiler vers le haut du groupe
...
/* Style pour les accordéons de correction */
